...
if [ -f "data/exam_words.db" ]; then
    ...
    if [ "$answer" != "n" ] && [ "$answer" != "N" ]; then
        ...
        (cd scripts && $PYTHON pdf_extractor.py "../英语真题源文件PDF" "../data/extracted")
        echo "[✓] Step 1/6 完成"

        echo "[*] ====== Step 2/6: 题型解析 ======"
        (cd scripts && $PYTHON exam_parser.py)
        echo "[✓] Step 2/6 完成"

        echo "[*] ====== Step 3/6: 合并词修复 ======"
        (cd scripts && $PYTHON word_splitter.py) || echo "[警告] 合并词修复失败，继续..."

        echo "[*] ====== Step 4/6: 手动修正 ======"
        (cd scripts && $PYTHON manual_fix.py) || echo "[警告] 手动修正失败，继续..."

        echo "[*] ====== Step 5/6: 单词提取与词频统计 ======"
        (cd scripts && $PYTHON word_processor.py)
        echo "[✓] Step 5/6 完成"

        echo "[*] ====== Step 6/6: 数据库构建 ======"
        (cd scripts && $PYTHON freq_builder.py)
        echo "[✓] Step 6/6 完成"
